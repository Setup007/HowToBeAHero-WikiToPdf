  ...
  this._router.findNode(item.key, function(err, contacts) {
    ...
    async.map(contacts, function(contact, done) {
      ...
      node._rpc.send(contact, message, done);
        ...
        var res = results.length && results[0] && JSON.parse(results[0].result.res).value;
...
Node.prototype._storeValidatedKeyValue = function(item, incomingMsg) {
  ...
  this._storage.put(item.key, JSON.stringify(item), function(err, res) {
    var contact = node._rpc._createContact(incomingMsg.params.contact);
    var message = new Message({
      error: err,
      result: { contact: node._self, res: res },
      id: incomingMsg.id
    });
